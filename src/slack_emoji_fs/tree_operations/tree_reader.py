...
from slack_emoji_fs.tree_operations.errors import IsDirectoryError, InvalidFileRangeError
...
class TreeReader:
    ...
    def list_directory(self, path: str) -> tuple[str, ...]:
        ...
        return tuple(resolved_directory.dirent_object.entries.keys())
    ...
    def read_file(self, path: str, *, offset: int = 0, size: int | None = None) -> bytes:
        ...
        if offset < 0:
            raise InvalidFileRangeError("Cannot read a negative offset.")
        if size is not None and size < 0:
            raise InvalidFileRangeError("Cannot read a negative size.")

        resolved_file = self._tree_navigator.resolve(self.snapshot.root_inode_id, path)
        resolved_file_inode = resolved_file.inode_object
        if not isinstance(resolved_file_inode, FileInodeObject):
            raise IsDirectoryError(f"Expected FileInodeObject but got {resolved_file_inode}")
    ...
    def read_file_inode(self, file_inode: FileInodeObject, *, offset: int = 0, size: int | None = None) -> bytes:
        ...
        if size is not None and size < 0:
            raise InvalidFileRangeError("Cannot read a negative size.")
        net_size = inode_size if size is None else min(inode_size, size)
        ...
        for chunk_index in range(starting_chunk, len(file_inode.chunks)):
            read_next_bytes = net_size - len(file_contents)
